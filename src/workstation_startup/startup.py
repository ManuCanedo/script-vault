...
def open_terminals():
    delay = 0.2
    screen_info = subprocess.check_output("xrandr | grep '*' | awk '{print $1}'", shell=True).decode().strip()
    ...
    subprocess.run(["wmctrl", "-r", ":ACTIVE:", "-e", f"0,0,0,{screen_width//2},{screen_height}"])

    t2 = subprocess.Popen(["gnome-terminal"])
    time.sleep(delay)  # Allow time for the window to open
    subprocess.run(["wmctrl", "-r", ":ACTIVE:", "-b", "remove,maximized_vert,maximized_horz"])
    subprocess.run(["wmctrl", "-r", ":ACTIVE:", "-e", f"0,{screen_width//2},0,{screen_width//2},{screen_height}"])
